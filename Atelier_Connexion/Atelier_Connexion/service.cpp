...
Service::Service(int id, int prix, int id_client, int id_employe, QString nom, QString description, QDate date)
{
    this->id = id;
...
{
    QString queryStr = "INSERT INTO PROJETCPP2A26.GS_SERVICE (ID_SERVICE, NOM, DESCRIPTION, PRIX, DATE_ESTIME, ID_CLIENT, ID_EMPLOYE) "
    ...
    if (query.exec()) {
        ...
        QString destinataire = "rmrm36961@gmail.com";  // Remplacez par l'email réel du destinataire
...
QSqlQueryModel * Service::afficher()
{
    ...
    model->setQuery("SELECT * FROM PROJETCPP2A26.GS_SERVICE");
...
bool Service::supprimer(int id)
{
    ...
    query.prepare("DELETE FROM PROJETCPP2A26.GS_SERVICE WHERE ID_SERVICE = :id");
    query.bindValue(":id", id);
...
bool Service::modifier()
{
    ...
    query.prepare("UPDATE PROJETCPP2A26.GS_SERVICE SET PRIX = :prix, NOM = :nom, DESCRIPTION = :description, DATE_ESTIME = :date , ID_CLIENT = :id_client, ID_EMPLOYE = :id_employe WHERE ID_SERVICE = :id");
...
{
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM PROJETCPP2A26.GS_SERVICE WHERE ID_SERVICE = :id");
    query.bindValue(":id", id);
